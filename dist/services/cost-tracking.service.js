...
export class CostTrackingService {
    repo;
    collectionsRepo;
    constructor(repo, collectionsRepo) {
        this.repo = repo;
    ...
    async copyFromPreviousYear(projectId, sourceYear, targetYear, createdBy) {
        const sourceCosts = await this.repo.getMonthlyCosts(projectId, sourceYear);
        const payloads = sourceCosts.map(cost => ({
            projectId,
            categoryId: cost.categoryId,
            year: targetYear,
            month: cost.month,
    ...
    // ---- Budget vs Actuals ----
    async getBudgetVsActuals(projectId, year) {
